...
def plot_MSEs(pred_dates, y, y_hat):
    ...
    for pred_date in pred_dates:
        ...
        if pred_date in y_hat.keys():
            ...
            MSE_tests_date[pred_date] = mean_squared_error(np.concatenate(y[pred_date].values()),
                                                           np.concatenate(y_hat[pred_date].values()))
    ...
    plt.plot(range(0, len(pred_dates)), MSE_tests_date.values(),
             label="MSE (" + str(np.round(np.mean(MSE_tests_date.values()), 3)) + ")")
